module ApiIterator
  ...
  def self.get_key_value(data_hash, key_string, type_expected)
  ...
  def self.get_array_match(data_array, search_term)
  ...
  def self.parse_items(json_hash)
  ...
  def self.parse_volumes(data_array)
  ...
  # Returns an array containing empty hash(es) if nothing found.
  def self.parse_sales(data_array)
    output = data_array.map do |data|
